...
void Renderer::setScissor(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
    ...
    // It seems that some menu rendering uses different scissor sizes
...
void Renderer::setD3DProjection(struct matrix* matrix)
{
    ...
    if(aspect_ratio >= AR_WIDESCREEN)
    {
        internalState.d3dProjectionMatrix[0] *= float(game_width) / wide_viewport_width;
        internalState.d3dProjectionMatrix[8] *= float(game_width) / wide_viewport_width;
    }
