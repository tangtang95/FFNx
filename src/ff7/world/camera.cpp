...
namespace ff7::world
{
    ...
    void update_world_camera_front(int current_key_input, int prev_key_input)
    {
        ...
        else// if (!highwind_view_and_square_pressed)
        {
            ...
            */
            ...
            camera.rotationOffset.x = std::min(std::max(camera.rotationOffset.x - rotationSpeed.x, -175.0f), -95.0f);
            camera.rotationOffset.y = std::remainder(camera.rotationOffset.y - rotationSpeed.y, 360.0f);

            *ff7_externals.world_camera_front_DFC484 = camera.rotationOffset.y * 4096 / 360.0f;
        }
        /*
    ...
    void update_world_camera(short world_camera_rotation_y)
    {
        ...
        if(candidateDist < camera.maxZoomDist && candidateDist > camera.minZoomDist)
        {
            camera.zoomOffset = camera.zoomOffset + camera.getZoomSpeed();
        }
        ...
        translation_matrix->pos_z = 10000 - camera.zoomOffset;

        //*ff7_externals.world_camera_delta_y_DE6A04 = -2000;
    }
