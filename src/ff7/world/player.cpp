    ...
    void update_player_and_handle_input()
    {
        ...
        if(*ff7_externals.world_is_control_enabled_DE6B5C && ff7_externals.world_get_unknown_flag_75335C())
        {
            ...
            if(camera_view_type == FRONT_VIEW || camera_view_type == HIGHWIND_VIEW || worldmap_type == SNOWSTORM)
            {
                ...
                {
                    bool is_movement_key_pressed = is_key_pressed(current_key_input, highwind_view_and_square_pressed ? ANY_DIRECTIONAL_KEY: CIRCLE);
                ...
                if(camera_view_type == HIGHWIND_VIEW && !highwind_view_and_square_pressed)
                {
                    ...
                    if(is_key_pressed(current_key_input, UP) && player_highwind_position.y > 500 && *ff7_externals.world_y_player_pos_flag_DE6A14 &&
                    ...
                    else if(is_key_pressed(current_key_input, DOWN) && player_highwind_position.y < *ff7_externals.world_highwind_height_lowerbound_DF5420 - 50 * movement_multiplier)
                    {
                        ff7_externals.world_add_y_pos_to_current_entity_761F22(50 * movement_multiplier);
                        vertical_delta = -10;
                    }
                ...
                if(worldmap_type == UNDERWATER)
                {
                    ...
                    if(is_key_pressed(current_key_input, UP) && player_submarine_position.y > -5000 && *ff7_externals.world_y_player_pos_flag_DE6A14)
                    {
                        ff7_externals.world_add_y_pos_to_current_entity_761F22(-30 * movement_multiplier);
                        vertical_delta = 10;
                    }

                    if(is_key_pressed(current_key_input, DOWN) && player_submarine_position.y < -3000)
                    {
                        ff7_externals.world_add_y_pos_to_current_entity_761F22(30 * movement_multiplier);
                        vertical_delta = -10;
                    }
                ...
                if(worldmap_type != SNOWSTORM)
                {
                    ...
                    int current_direction = (camera_view_type != FRONT_VIEW || worldmap_type == UNDERWATER) ? 0 : player_direction;
